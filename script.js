...
function applyConfig() {
    if (typeof CONFIG !== 'undefined') {
        ...
        // 设置区域标题
        setConfigText('demo-video-title', CONFIG.content.sections.demoVideo.title);
        setConfigText('demo-video-subtitle', CONFIG.content.sections.demoVideo.subtitle);
        setConfigText('organizations-title', CONFIG.content.sections.organizations.title);
        setConfigText('organizations-subtitle', CONFIG.content.sections.organizations.subtitle);
        setConfigText('characters-title', CONFIG.content.sections.characters.title);
        setConfigText('characters-subtitle', CONFIG.content.sections.characters.subtitle);
        setConfigText('about-title', CONFIG.content.sections.about.title);
        setConfigText('about-subtitle1', CONFIG.content.sections.about.subtitle);
...
function updateDemoVideoCarousel() {
    ...
    if (cards.length === 1) {
        ...
        if (currentVideo && currentVideo.video) {
            ...
            if (iframe) {
                iframe.src = currentVideo.video;
                console.log(`设置演示视频 iframe src: ${currentVideo.video}`);
            }
            ...
            if (video) {
                ...
                console.log(`设置演示视频 video src: ${currentVideo.video}`);
            }
        }
    } else {
        // 如果有多个卡片，使用原来的逻辑
...
function updateOrgCarousel() {
    ...
    if (cards.length === 1) {
        ...
        if (currentOrg) {
            // 更新图片
            const img = card.querySelector('.org-image img');
            ...
            // 更新标题
            const title = card.querySelector('.org-content h3');
            if (title) {
                title.textContent = currentOrg.name;
            }
            
            // 更新描述
            const description = card.querySelector('.org-content p');
            if (description) {
                description.textContent = currentOrg.description;
            }
            
            // 更新统计信息
            const stats = card.querySelector('.org-stats');
            if (stats) {
                stats.innerHTML = `
...
function renderCharacterList() {
    ...
    currentPageChars.forEach((character, pageIndex) => {
        ...
        item.innerHTML = `
            ...
            <span class="fade-in-bottom">${character.name}</span>
        `;
...
function playAnimation() {
    ...
    var element1 = document.getElementById('character-name');
    element1.classList.add('tracking-in-expand-fwd-top'); 
    setTimeout(() => {
      element1.classList.remove('tracking-in-expand-fwd-top'); 
    }, 500);

    var element2 = document.getElementById('character-title');
    element2.classList.add('tracking-in-expand'); 
    setTimeout(() => {
      element2.classList.remove('tracking-in-expand'); 
    }, 500);

    var element3 = document.getElementById('character-tags');
    element3.classList.add('scale-in-hor-left'); 
    setTimeout(() => {
      element3.classList.remove('scale-in-hor-left'); 
    }, 500);

    var element4 = document.getElementById('character-desc');
    element4.classList.add('focus-in-contract-bck'); 
    setTimeout(() => {
      element4.classList.remove('focus-in-contract-bck'); 
    }, 500);
...
function applyAboutConfig() {
    if (!CONFIG.content.about) return;
    ...
    setConfigText('about-subtitle3', about.subtitle3);
...
function applyFooterConfig() {
    if (!CONFIG.content.footer) return;
    
    const footer = CONFIG.content.footer;
    
    // 设置主要区域
    if (footer.sections.main) {
        const mainTitle = document.querySelector('.footer-section h3');
        const mainDesc = document.querySelector('.footer-section p');
        if (mainTitle) mainTitle.textContent = footer.sections.main.title;
        if (mainDesc) mainDesc.textContent = footer.sections.main.description;
    }
    
    // 设置加入我们区域
    if (footer.sections.join) {
        const joinTitle = document.querySelector('.footer-section:nth-child(2) h4');
        const joinDesc = document.querySelector('.footer-section:nth-child(2) p');
        if (joinTitle) joinTitle.textContent = footer.sections.join.title;
        if (joinDesc) joinDesc.textContent = footer.sections.join.description;
    }
    
    // 设置关注我们区域
    if (footer.sections.follow) {
        const followTitle = document.querySelector('.footer-section:nth-child(3) h4');
        if (followTitle) followTitle.textContent = footer.sections.follow.title;
        
        // 设置社交链接
        const socialLinksContainer = document.querySelector('.social-links');
        if (socialLinksContainer && footer.sections.follow.socialLinks) {
            socialLinksContainer.innerHTML = '';
            footer.sections.follow.socialLinks.forEach(link => {
